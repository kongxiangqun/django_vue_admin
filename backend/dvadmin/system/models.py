...
class CaseHierarchy(CoreModel):
    ...
    name = models.CharField(max_length=64, verbose_name="测试用例分类名称", help_text="测试用例分类名称")
    login_type = models.IntegerField(default=1, choices=LOGIN_TYPE_CHOICES, verbose_name="登录类型", help_text="登录类型")

    class Meta:
        db_table = table_prefix + "system_case_hierarchy"
        verbose_name = '测试用例目录层级表'
        verbose_name_plural = verbose_name
        ordering = ('sort',)


class TestCase(CoreModel):
    number = models.CharField(max_length=32, verbose_name="测试用例编号", help_text="测试用例编号")
    sort = models.IntegerField(default=1, verbose_name="显示排序", help_text="显示排序")
    Scope_applicaton = models.CharField(max_length=200, verbose_name="试用范围", help_text="试用范围")
    standard = models.CharField(max_length=200, verbose_name="所属标准", help_text="所属标准")
    scene_image = models.CharField(max_length=255, verbose_name="场景图", null=True, blank=True, help_text="场景图")
    select_count = models.IntegerField(verbose_name="查看次数", help_text="查看次数")
    export_count = models.IntegerField(verbose_name="导出次数", help_text="导出次数")
    parent = models.ForeignKey(to='CaseHierarchy', on_delete=models.CASCADE, default=None, verbose_name="标题层级",
                               db_constraint=False, null=True, blank=True, help_text="标题层级")
    test_plan = models.ForeignKey(to='TestPlan', on_delete=models.CASCADE, default=None, verbose_name="测试计划",
                               db_constraint=False, null=True, blank=True, help_text="测试计划")

    class Meta:
        db_table = table_prefix + "system_test_case"
        verbose_name = '测试用例表'
        verbose_name_plural = verbose_name
        ordering = ('sort',)

class TestPlan(CoreModel):
    name = models.CharField(max_length=32, verbose_name="计划名称", help_text="计划名称")
    test_count = models.IntegerField(verbose_name="测试任务数量", help_text="测试任务数量")
    test_progress = models.IntegerField(verbose_name="测试进度", help_text="测试进度")
    select_count = models.IntegerField(verbose_name="查看次数", help_text="查看次数")
    export_count = models.IntegerField(verbose_name="导出次数", help_text="导出次数")


    class Meta:
        db_table = table_prefix + "system_test_plan"
        verbose_name = '测试计划表'
        verbose_name_plural = verbose_name
        ordering = ('sort',)
